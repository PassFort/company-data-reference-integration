...
def test_run_search_protected(session, auth):
    ...
    r = session.post(
        "http://app/search", json={}, headers={"date": old_date}, auth=auth()
    )
    assert r.status_code == 401
...
def test_run_demo_search(session, auth):
    ...
    with open(
        os.path.join(
            os.path.dirname(__file__), "../static/demo_results/MANY_HITS.json"
        ),
        "r",
    ) as file:
    ...
    assert json.dumps(res["search_output"], sort_keys=True, indent=2) == json.dumps(
        demo_data["search_output"], sort_keys=True, indent=2
    )

    ...
    if passfort_data.get('input_data'):
        ...
        if passfort_data['input_data'].get('personal_details'):
            ...
            if passfort_data['input_data']['personal_details'].get('gender'):
                lexisnexis_pkg['InstantIDRequest']['SearchBy']['Gender'] = passfort_data['input_data']['personal_details']['gender'].upper()

            # Check SSN ( Segurity Social Number )
        ...
        if passfort_data['input_data'].get('address_history'):
            for idx, address in enumerate(passfort_data['input_data']['address_history'][:1]):
                ...
                if address_to_check.get('street_number'):
                    address['StreetAddress1'] = address.get('StreetAddress1', '') + ' ' + str(address_to_check['street_number'])
                ...
                if address_to_check.get('premise'):
                    address['StreetAddress1'] = address.get('StreetAddress1', '') + ' ' + str(address_to_check['premise'])

                if address_to_check.get('subpremise'):
                    address['StreetAddress1'] = address.get('StreetAddress1', '') + ' ' + str(address_to_check['subpremise'])
        ...
        if passfort_data['input_data'].get('contact_details'):
            if passfort_data['input_data']['contact_details'].get('email'):
                lexisnexis_pkg['InstantIDRequest']['SearchBy']['Email'] = passfort_data['input_data']['contact_details']['email']

            if passfort_data['input_data']['contact_details'].get('phone_number'):
                lexisnexis_pkg['InstantIDRequest']['SearchBy']['HomePhone'] = passfort_data['input_data']['contact_details']['phone_number']
                lexisnexis_pkg['InstantIDRequest']['SearchBy']['WorkPhone'] = passfort_data['input_data']['contact_details']['phone_number']
...
def lexisnexis_to_passfort_data(lexisnexis_response_data):
    ...
    if lexisnexis_response_data['status'] in [401, 403]:
        ...
        response_body['errors'].append({
                'code': 303, 
                'message': f"Provider Error: UNKNOW ERROR: {lexisnexis_response_data['body']}"})
        response_body['output_data']['decision'] = 'ERROR'
